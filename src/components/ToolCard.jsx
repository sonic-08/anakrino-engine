import { Sparkles, Star, ArrowRight, Scale, ThumbsUp, ThumbsDown, MessageCircle } from 'lucide-react';
...
export default function ToolCard({
  ...
  return (
    <Card3D onClick={() => onSelect(tool)} className="cursor-pointer flex">
      ...
      <div className="w-full bg-emerald-500/[0.08] border border-emerald-500/20 p-5 sm:p-6 rounded-2xl mb-6 flex flex-col justify-center relative overflow-hidden group-hover:bg-emerald-500/[0.14] group-hover:border-emerald-500/30 transition-colors shrink-0">
        <div className="flex items-center gap-2 mb-2">
          <Sparkles className="w-4 h-4 text-emerald-400" />
          <h4 className="text-[0.7rem] font-extrabold text-emerald-400 uppercase tracking-widest">
            Why For You
          </h4>
        </div>
        <p className="text-sm sm:text-base text-emerald-100/90 font-medium leading-relaxed line-clamp-3">
      ...
      <div className="mb-4 pr-14">
        <h3 className="font-heading text-2xl sm:text-3xl font-bold text-white mb-3 tracking-tight leading-snug line-clamp-2 group-hover:text-emerald-300 transition-colors">
          {tool.name}
        </h3>
      ...
      <div className="w-full flex items-center gap-3 mt-6 pt-4 border-t border-white/[0.06] shrink-0">
        <div className="w-full h-12 bg-white/[0.04] border border-white/10 rounded-xl flex items-center justify-center text-sm font-bold text-emerald-400 group-hover:bg-emerald-600 group-hover:border-emerald-500 group-hover:text-white transition-all duration-300 shadow-sm">
