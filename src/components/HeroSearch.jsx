    ...
    >
      ...
      <div className="w-full max-w-3xl relative z-20 mb-8">
        <form onSubmit={onSubmit} className="relative w-full">
          <div className="relative flex items-center bg-white/[0.04] border border-white/10 rounded-[2.2rem] p-2.5 shadow-[0_30px_60px_rgba(0,0,0,0.6),inset_0_2px_10px_rgba(255,255,255,0.05)] backdrop-blur-3xl focus-within:bg-white/[0.06] focus-within:border-emerald-500/50 focus-within:shadow-[0_0_50px_rgba(16,185,129,0.25)] transition-all duration-400 group">
            <div className="w-12 h-12 bg-white/5 rounded-2xl flex items-center justify-center ml-2 border border-white/5 group-focus-within:bg-emerald-500/10 group-focus-within:border-emerald-500/30 transition-colors shrink-0">
        ...
        <div className="flex flex-wrap justify-center gap-2">
          {DISCOVERY_TAGS.map((tag, idx) => (
            <button
              ...
              className="px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold bg-white/[0.03] hover:bg-emerald-600/20 text-zinc-300 hover:text-white border border-white/10 hover:border-emerald-500/40 transition-all cursor-pointer shadow-sm hover:scale-[1.02] active:scale-95"
            >
              {tag.label}
            </button>
          ))}
